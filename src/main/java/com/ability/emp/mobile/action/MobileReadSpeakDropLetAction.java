...
import com.ability.emp.mobile.entity.bean.ReadSpeakBean;
import com.ability.emp.mobile.entity.vo.DropLetButtonVo;
import com.ability.emp.mobile.entity.vo.ReadSpeakDropLetVo;
...
@CrossOrigin//解决跨域请求
@Controller
@RequestMapping("/mobile/getReadSpeak")
public class MobileReadSpeakDropLetAction {
	...
	@Resource
	private MobileReadSpeakDropLetService mobileReadSpeakDropLetService;
	
	ObjectMapper objectMapper = new ObjectMapper();
	
	
	
	@RequestMapping("/getReadSpeak/{dropLetId}/{dropLetConfTypeId}")
	@ResponseBody
	public String getReadSpeak(@PathVariable("dropLetId") String dropLetId,@PathVariable("dropLetConfTypeId") String dropLetConfTypeId) throws Exception {
		
		List<DropLetButtonVo> list2 = new ArrayList<DropLetButtonVo>();
		Map<String,Object> map = new HashMap<String,Object>();
		MobileReadSpeakDropLetEntity me = new MobileReadSpeakDropLetEntity();
		...
		MobileReadSpeakDropLetEntity readSpeakEntity = mobileReadSpeakDropLetService.getReadSpeakData(me);
		ReadSpeakDropLetVo rv = new ReadSpeakDropLetVo();
		if(readSpeakEntity!=null){
			rv.setImage(SysConstant.SERVICE_HOST+readSpeakEntity.getImage());
			String[] tempsentence = readSpeakEntity.getSentence().split(" ");
			Object[] sentence = new Object[tempsentence.length];
			for(int i=0;i<tempsentence.length;i++){
				ReadSpeakBean rb = new ReadSpeakBean();
				rb.setTit(tempsentence[i]);
				rb.setFlag(false);
				
				sentence[i]=rb;
			}
			rv.setSentence(sentence);
			rv.setReadtype(readSpeakEntity.getReadtype());
			if(readSpeakEntity.getReadtype().equals("speak")){
				rv.setAudio(SysConstant.SERVICE_HOST+readSpeakEntity.getAudio());
			}
			rv.setReladropletid(readSpeakEntity.getReladropletid());
			rv.setReladropletconftypeid(readSpeakEntity.getReladropletconftypeid());
			
		}
		
		md.setId(readSpeakEntity.getReladropletid());
		MobileDropLetEntity mo = mobileDropLetService.getDropLetByID(md);
		if(mo!=null){
			rv.setDropLetLink(mo.getDropletlink());
		}
		 ...
		 */
		if(readSpeakEntity!=null 
		   && readSpeakEntity.getDropletconftypeid()!=null
		   && !"".equals(readSpeakEntity.getDropletconftypeid())
		){
			MobileDropLetConfTypeEntity mde = new MobileDropLetConfTypeEntity();
			mde.setDropletconftype(readSpeakEntity.getDropletconftypeid());//确定是哪个位置
			mde.setDropletid(readSpeakEntity.getDropletid());//确定是哪个droplet
